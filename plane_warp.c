...
static int g_fast = 0;
static int g_flat = 0;              // flat-view 2-pass NN decoder
...
int g_singleshot = 1;
// 4-rotation flag: wraps any --decode-* mode with 4 rotations
...
int solve_plane_flat(int r, int s, uint8_t *syn, uint8_t *out); // 2-pass NN decoder
...
void canonicalize(int r, int s, uint8_t *corr) {
    if((r & 1) || (s & 1)) return;
    int hr = r/2, hs = s/2;
    // Canonicalize each parity sub-lattice independently.
    // The kernel of (1+u)(1+v) on an hr×hs block is generated by row flips
    // and column flips (dimension hr+hs-1). We pick the kernel element that
    // gives the minimum-weight correction while making the result deterministic.
    //
    // Determinism is achieved by applying kernel flips in a fixed order:
    // greedily accept any column flip (b=hs-1..0) or row flip (a=hr-1..1)
    // and re-sweep until stable. This always converges to the same minimum
    // because the order is fixed and the cost is uniform.
    //
    // We also try both sweep orders and take the better result.
    uint8_t *blk = malloc((size_t)hr*hs);
    uint8_t *best_blk = malloc((size_t)hr*hs);
    if(!blk || !best_blk) { free(blk); free(best_blk); return; }
    for(int si=0; si<2; si++) for(int sj=0; sj<2; sj++) {
        // Extract this parity class block
        for(int a=0;a<hr;a++) for(int b=0;b<hs;b++)
            blk[a*hs+b] = corr[((si+2*a)%r)*s + ((sj+2*b)%s)];
        memcpy(best_blk, blk, (size_t)hr*hs);
        int best_w = 0;
        for(int p=0;p<hr*hs;p++) best_w += blk[p];

        // Try both sweep orders
        for(int order=0; order<2; order++) {
            uint8_t *tmp = malloc((size_t)hr*hs);
            if(!tmp) continue;
            memcpy(tmp, blk, (size_t)hr*hs);
            int changed = 1;
            while(changed) {
                changed = 0;
                if(order == 0) {
                    for(int b=hs-1; b>=0; b--) {
                        int w0=0, w1=0;
                        for(int a=0;a<hr;a++){ int v=tmp[a*hs+b]; w0+=v; w1+=(v^1); }
                        if(w1 < w0) { for(int a=0;a<hr;a++) tmp[a*hs+b]^=1; changed=1; }
                    }
                    for(int a=hr-1; a>=0; a--) {
                        int w0=0, w1=0;
                        for(int b=0;b<hs;b++){ int v=tmp[a*hs+b]; w0+=v; w1+=(v^1); }
                        if(w1 < w0) { for(int b=0;b<hs;b++) tmp[a*hs+b]^=1; changed=1; }
                    }
                } else {
                    for(int a=hr-1; a>=0; a--) {
                        int w0=0, w1=0;
            ...
            int w = 0;
            for(int p=0;p<hr*hs;p++) w += tmp[p];
            if(w < best_w) { best_w = w; memcpy(best_blk, tmp, (size_t)hr*hs); }
            free(tmp);
        }
        // Write back the best representative
        for(int a=0;a<hr;a++) for(int b=0;b<hs;b++)
            corr[((si+2*a)%r)*s + ((sj+2*b)%s)] = best_blk[a*hs+b];
    }
    free(blk); free(best_blk);
}
...
static void solve_plane_5d_mv(int r, int s, uint8_t *syn, uint8_t *syn_mv, uint8_t *out) {
    ...
    for(int si=0;si<2;si++) for(int sj=0;sj<2;sj++){
        ...
        // Joint cost: |E| + Σ_f |vline(E) ⊕ Ec_v_f|*2 + |hline(E) ⊕ Ec_h_f|*2
        // Bundle cost: |E| + Σ_f |aggregate(E) ⊕ Ec_f|
        // Face aggregates (2×2 XOR) are invariant under kernel row/col flips,
        // so the penalty is constant across kernel choices. The enumeration
        // below only minimizes weight; the greedy descent uses full BCOST.
        ...
        // Exhaustive kernel enumeration for small blocks (hr ≤ 12 → 4096 row masks).
        // Row/col flips are the entire kernel of (1+x)(1+y) and leave face
        // aggregates invariant, so we only minimize weight here.
        if(hr <= 12) {
            int best_rmask=0, best_cmask=0;
            double best_wt = 1e100;
            for(int rmask=0; rmask < (1<<hr); rmask++) {
                int cmask = 0;
                for(int b=0; b<hs; b++) {
                    int ones=0;
                    for(int a=0; a<hr; a++) {
                        int val = E[SEC(a,b)] ? 1 : 0;
                        if(val ^ ((rmask>>a)&1)) ones++;
                    }
                    if(ones > hr - ones) cmask |= (1<<b);
                }
                double wt = 0;
                for(int a=0; a<hr; a++) for(int b=0; b<hs; b++) {
                    int flip = ((rmask>>a)&1) ^ ((cmask>>b)&1);
                    if(E[SEC(a,b)] ^ flip) wt += W[SEC(a,b)];
                }
...
int solve_plane(int r, int s, uint8_t *syn, uint8_t *out) {
    ...
    #define SEC(a,b) ((a)*hs + (b))
    int sz = hr * hs;
    uint8_t *S=calloc(MAX_N,1), *E=calloc(MAX_N,1), *best_E=calloc(MAX_N,1);
    ...
    // Each block: 2 corner seeds × forward 2D prefix XOR, then column/row
    // descent (strict majority), then 4 logical patterns (row-0 / column-0
    // flips + descent), then K_fix column-1 refinement.
    // Uses per-block independent best selection.
    for(int si=0; si<2; si++) for(int sj=0; sj<2; sj++) {
        int best_sec_wt = hr*hs + 1;
        memset(best_E, 0, sz);
        for(int a=0;a<hr;a++) for(int b=0;b<hs;b++) {
            int q = ((si+2*a)%r)*s + ((sj+2*b)%s);
            S[SEC(a,b)] = syn[q];
        ...
        for(int c0=0; c0<2; c0++) {
            ...
            for(int a=0;a<hr-1;a++) for(int b=0;b<hs-1;b++)
                E[SEC(a+1,b+1)] = S[SEC(a,b)] ^ E[SEC(a,b)] ^ E[SEC(a+1,b)] ^ E[SEC(a,b+1)];
            for(int b=1;b<hs;b++) {
                int w=0; for(int a=0;a<hr;a++) if(E[SEC(a,b)]) w++;
                if(w > hr/2) for(int a=0;a<hr;a++) E[SEC(a,b)] ^= 1;
            }
            for(int a=1;a<hr;a++) {
                int w=0; for(int b=0;b<hs;b++) if(E[SEC(a,b)]) w++;
                if(w > hs/2) for(int b=0;b<hs;b++) E[SEC(a,b)] ^= 1;
            }
            for(int b=1;b<hs;b++) {
                int w=0; for(int a=0;a<hr;a++) if(E[SEC(a,b)]) w++;
                if(w > hr/2) for(int a=0;a<hr;a++) E[SEC(a,b)] ^= 1;
            ...
            // 4 logical patterns
            uint8_t seed_state[MAX_N]; memcpy(seed_state, E, sz);
            for(int log=0;log<4;log++) {
                memcpy(E, seed_state, sz);
                ...
                if(wt < best_sec_wt) { best_sec_wt = wt; memcpy(best_E, E, sz); }
            }
        }
        // K_fix per block
        int L = 0;
        for(int b=0;b<hs;b++) L ^= best_E[SEC(0,b)];
        for(int a=1;a<hr;a++) L ^= best_E[SEC(a,0)];
        if(L) for(int a=0;a<hr;a++) best_E[SEC(a,1)] ^= 1;
        for(int b=1;b<hs;b++) {
            int w=0; for(int a=0;a<hr;a++) if(best_E[SEC(a,b)]) w++;
            if(w > hr/2) for(int a=0;a<hr;a++) best_E[SEC(a,b)] ^= 1;
        }
        for(int a=0;a<hr;a++) for(int b=0;b<hs;b++)
            if(best_E[SEC(a,b)]) out[((si+2*a)%r)*s + ((sj+2*b)%s)] ^= 1;
    }
    #undef SEC
    free(S); free(E); free(best_E); free(W);
...
// ---- 4D rotation helpers (exported for decoder.py ctypes) ----
int ZW_MAT[6][4] = {
    {1,0,0,1},  // 0: identity
    {0,1,1,0},  // 1: swap X↔Z
...
// Apply forward 4D rotation: out[rot(i,j)] = syn[i][j]
void rot_4d_fwd(int r, int s, uint8_t *syn, uint8_t *out, int dx, int dy, int mi) {
...
void rot_4d_inv(int r, int s, uint8_t *corr, uint8_t *out, int dx, int dy, int mi) {
    int m00=ZW_MAT[mi][0], m01=ZW_MAT[mi][1], m10=ZW_MAT[mi][2], m11=ZW_MAT[mi][3];
    int n=r*s; memset(out,0,n);
    for(int qi=0; qi<r; qi++) for(int qj=0; qj<s; qj++) {
        int it = (qi + dx) % r;
...
// ============================================================
// FLAT-VIEW DECODER — (1+x²)(1+y²) via two passes of full-grid
// (1+x)(1+y) nearest-neighbor decode, exploiting the Frobenius
// square root  (1+x²)(1+y²) = ((1+x)(1+y))².
//
// Step 1:  S_nn ← nn⁻¹(S_stride2)   — find syndrome vector S_nn
//           such that (1+x)(1+y)·S_nn = S_stride2.
// Step 2:  E ← nn⁻¹(S_nn)           — find data correction E
//           such that (1+x)(1+y)·E = S_nn.
//
// Combined:  (1+x²)(1+y²)·E = S_stride2.  Distance = min(r,s) —
// double the independent-block decoder's distance of min(r,s)/2.
// ============================================================
static void nn_pass(int r, int s, uint8_t *syn, uint8_t *out, int do_fixup) {
    int n = r * s;
    memset(out, 0, n);
    // 2D prefix XOR with zero boundary: out[0,:]=out[:,0]=0
    for(int i=0;i<r-1;i++) for(int j=0;j<s-1;j++)
        out[(i+1)*s+(j+1)] = syn[i*s+j] ^ out[i*s+j] ^ out[(i+1)*s+j] ^ out[i*s+(j+1)];
    // Column descent (j=1..s-1): flip if strict majority of r rows
    for(int j=1;j<s;j++) {
        int w=0; for(int i=0;i<r;i++) if(out[i*s+j]) w++;
        if(w > r/2) for(int i=0;i<r;i++) out[i*s+j] ^= 1;
    }
    // Row descent (i=1..r-1): flip if strict majority of s columns
    for(int i=1;i<r;i++) {
        int w=0; for(int j=0;j<s;j++) if(out[i*s+j]) w++;
        if(w > s/2) for(int j=0;j<s;j++) out[i*s+j] ^= 1;
    }
    // Column re-pass (row descent may have broken column optimality)
    for(int j=1;j<s;j++) {
        int w=0; for(int i=0;i<r;i++) if(out[i*s+j]) w++;
        if(w > r/2) for(int i=0;i<r;i++) out[i*s+j] ^= 1;
    }
    if(do_fixup) {
        // K_fix = column 1 if Z_L1Z_L2 = 1
        int L = 0;
        for(int j=0;j<s;j++) L ^= out[j];
        for(int i=1;i<r;i++) L ^= out[i*s];
        if(L) for(int i=0;i<r;i++) out[i*s+1] ^= 1;
        int w=0; for(int i=0;i<r;i++) if(out[i*s+1]) w++;
        if(w > r/2) for(int i=0;i<r;i++) out[i*s+1] ^= 1;
    }
}
