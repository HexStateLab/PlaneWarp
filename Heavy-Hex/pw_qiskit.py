...
class PlaneWarp:
    ...
    def _build_stab_group(self, r=6, s=8):
        ...
        Enumerates the 8-dim stabilizer group (generated by weight-4
        X-stabilizers on each 2×2 block) and caches the result.
        """
        if PlaneWarp._STAB_GROUP is not None:
            return PlaneWarp._STAB_GROUP[:256].reshape(-1, r, s)
        # 12 weight-4 X stabilizers (one per 2×2 block of the 6×8 grid)
        gens = []
        for i in range(r // 2):
            for j in range(s // 2):
                g = np.zeros((r, s), dtype=np.uint8)
                g[2*i, 2*j] = 1
                g[2*i, (2*j+2) % s] = 1
                g[(2*i+2) % r, 2*j] = 1
                g[(2*i+2) % r, (2*j+2) % s] = 1
                gens.append(g.ravel())
        # Gaussian elimination on 12×48 → pick 8 independent rows
        A = np.array(gens, dtype=np.uint8)
        indep = []
        for row in range(12):
            if len(indep) >= 8:
                break
            v = A[row].copy()
            for pv in indep:
                leader = int((pv != 0).argmax())
                if v[leader]:
                    v ^= pv
            if v.any():
                indep.append(v)
        # Enumerate all 256 linear combinations
        all_s = []
        for mask in range(256):
            s = np.zeros(r * s, dtype=np.uint8)
            for bit in range(8):
                if mask >> bit & 1:
                    s ^= indep[bit]
            all_s.append(s)
    ...
    def refine_min_weight(self, C):
        ...
        Searches all 256 stabilizer elements S and returns C_min = C ⊕ S
        with the smallest Hamming weight. This is the ML-optimal correction
        for CSS codes under independent noise.
        """
        r, s = C.shape
        stab = self._build_stab_group(r, s)
        c_flat = C.ravel()
        best = c_flat.copy()
        best_wt = int(best.sum())
        for si in range(256):
            cand = c_flat ^ stab[si].ravel()
            w = int(cand.sum())
            if w < best_wt:
                best = cand
                best_wt = w
