...
// ---- Multi-origin wrapper (experimental) ----
// Hypothesis: the entropy well is a coordinate singularity caused by
// privileging one warp origin. Decode from four translated charts and
// keep the lightest solution.
int solve_plane_multiorigin(int r, int s, uint8_t *syn, uint8_t *out) {
    int n=r*s;
    uint8_t best[MAX_N];
    double best_wt=n+1.0;

    int ox[4] = {0, r/4, r/2, (3*r)/4};
    int oy[4] = {0, s/4, s/2, (3*s)/4};

    for(int k=0;k<4;k++) {
        uint8_t rot_syn[MAX_N], cand[MAX_N];

        for(int i=0;i<r;i++) for(int j=0;j<s;j++) {
            int si=(i+ox[k])%r;
            int sj=(j+oy[k])%s;
            rot_syn[i*s+j]=syn[si*s+sj];
        }

        solve_plane_fast(r,s,rot_syn,cand);
        ...
        for(int i=0;i<r;i++) for(int j=0;j<s;j++) {
            int di=(i+ox[k])%r;
            int dj=(j+oy[k])%s;
            unrot[di*s+dj]=cand[i*s+j];
        }

        double wt=0;
        for(int q=0;q<n;q++) if(unrot[q]) wt+=cost_map[q];
        if(wt<best_wt) {
            best_wt=wt;
            memcpy(best,unrot,n);
        }
        ...
        for(int mi=0;mi<3;mi++) {
            ...
            for(int wi=0;wi<16;wi++) {
                ...
                for(int t=0;t<trials;t++) {
                    ...
                    solve_plane_multiorigin(r,s,syn,dec);
                    uint8_t diff[MAX_N];
